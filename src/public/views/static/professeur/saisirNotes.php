...
<main class="container ">
    <div class="w-50 mx-auto mt-5 fs-4 text-center p-2 text-white bg-danger" style="border-radius:10px;">Choisir la
        <span class=" text-white" style="letter-spacing: 0.8px;">filiere </span>
    </div>
    <div class=" mt-5 mx-auto w-50">
        <?php foreach ($filiers as $filiere) : ?>
            ...
            ?>
            <details>
                <summary class=" mt-3 p-3 text-white" style="background:#183258;border-radius:10px 10px 0 0;"><?= $filiere['nom_filiere'] ?></summary>
                <div class=" accordion-content  text-danger fs-4">
                    <?php foreach ($modulesFiliere as $module) : ?>
                        <div class="font py-3" style="background:radial-gradient(788px at 0.7% 3.4%, rgb(164, 231, 192) 0%, rgb(255, 255, 255) 90%);">
                            ...
                            <?= $module['nom_modules'] ?><span class="ms-4 ">Saisir</span><a href="./notes.php?filiere=<?= $module['nom_filiere'] ?>&module=<?= $module['nom_modules'] ?>&id=<?= $module['id'] ?>"><strong class="text-danger">(ici)</strong></a>
                        </div>
                    <?php endforeach; ?>
                </div>
            </details>
