                ...
                <?php foreach ($filiers as $key => $f) : ?>
                    <div class="tab-pane  fade <?= $key === 0 ? 'show active' : '' ?>" id="nav-<?= $f['id'] ?>" role="tabpanel" aria-labelledby="nav-<?= $f['id'] ?>-tab">
                        <div class="p-3" style="border-radius: 0 0 10px 10px;">
                            <i class="d-block mt-4 pb-4 ms-4 text-success fs-3 bi bi-bookmark-check-fill">
                                <span style="font-size: 20px;" class=" ms-2 text-dark">
                                    Consulter
                                    <a href="./voirLalistePromo.php?filiere=<?= $f['nom_filiere'] ?>&anne=<?= $ann_univ ?>&semestre=1">
                                        <i class="ms-3 text-danger fs-2 bi bi-folder-symlink-fill"></i>
                                    </a>
                                </span>
                            </i>
                        </div>
